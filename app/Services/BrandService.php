...
class BrandService
{
    ...
    {
        $data = [ 'seo_links' => $request->seo_links , 'seo_scripts' => $request->seo_scripts,'category_id' => $request->category_id, 'video_link' => $request->video_link];
        ...
        foreach ($request->title as $key => $value) {
            $translate_model::create(['slug' => Str::slug($request->title[$key]),'meta_keywords' => $request->meta_keywords[$key] ,'meta_description' => $request->meta_description[$key] , 'seo_title' => $request->seo_title[$key],'brand_id' => $translate->id, 'lang_key' => $key, 'title' => $value]);
        }
    ...
    public function simple_update($brand, $translate_model, $images_model, $request, $id)
    {
        ...
        $brand->update($data);
        foreach ($request->title as $key => $value) {
            $translation = $translate_model::where('brand_id', $id)->where('lang_key', $key)->first();
            if ($translation) {
                $translation->update(['slug' => Str::slug($request->title[$key]),'meta_keywords' => $request->meta_keywords[$key] ,'meta_description' => $request->meta_description[$key] , 'seo_title' => $request->seo_title[$key],'brand_id' => $id, 'lang_key' => $key, 'title' => $value]);
            } else {

                $translate_model::create(['slug' => Str::slug($request->title[$key]),'meta_keywords' => $request->meta_keywords[$key] ,'meta_description' => $request->meta_description[$key] , 'seo_title' => $request->seo_title[$key],'brand_id' => $id, 'lang_key' => $key, 'title' => $value]);
            }
        }
