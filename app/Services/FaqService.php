...
class FaqService
{
    public function simple_create($main_model, $translate_model, $request)
    {
        ...
        foreach ($request->question as $key => $value) {
            $translate_model::create(['faq_id' => $translate->id, 'lang_key' => $key, 'question' => $value, 'answer' => $request->answer[$key]]);
        }
    ...
    public function simple_update($category, $translate_model, $request, $id)
    {
        ...
        foreach ($request->question as $key => $value) {
            $translation = $translate_model::where('faq_id', $id)->where('lang_key', $key)->first();
            if ($translation) {
                $translation->update(['tag' => $value]);
            } else {
                $translate_model::create(['faq_id' => $id, 'lang_key' => $key, 'question' => $value, 'answer' => $request->answer[$key]]);
            }
        }
