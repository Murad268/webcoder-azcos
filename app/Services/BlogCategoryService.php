...
class BlogCategoryService
{
    ...
    {
        $data = [ 'seo_links' => $request->seo_links , 'seo_scripts' => $request->seo_scripts ];
        // Create the main model
        $translate = $main_model::create($data);
        // Create translations
        foreach ($request->title as $key => $value) {
            $translate_model::create(['slug' => Str::slug($request->title[$key]),'meta_keywords' => $request->meta_keywords[$key] ,'meta_description' => $request->meta_description[$key] , 'seo_title' => $request->seo_title[$key] ,'blog_category_id' => $translate->id,'lang_key' => $key, 'title' => $value]);
        }
    ...
    public function simple_update($category, $translate_model, $request, $id)
    {
        ...
        // Update translations
        foreach ($request->title as $key => $value) {
            $translation = $translate_model::where('blog_category_id', $id)->where('lang_key', $key)->first();
            if ($translation) {
                $translation->update(['slug' => Str::slug($request->title[$key]), 'meta_keywords' => $request->meta_keywords[$key] ,'meta_description' => $request->meta_description[$key] , 'seo_title' => $request->seo_title[$key] ,'blog_category_id' => $id, 'lang_key' => $key, 'title' => $value]);
            } else {
                $translate_model::create(['slug' => Str::slug($request->title[$key]),'meta_keywords' => $request->meta_keywords[$key] ,'meta_description' => $request->meta_description[$key] , 'seo_title' => $request->seo_title[$key] ,'blog_category_id' => $id, 'lang_key' => $key,'title' => $value]);
            }
        }
