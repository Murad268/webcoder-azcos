...
class TeamService
{
    public function simple_create($main_model, $translate_model, $images_model, $request)
    {
        ...
        foreach ($request->title as $key => $value) {
            $translate_model::create(['team_id' => $translate->id,'lang_key' => $key, 'title' => $value, 'position' => $request->position[$key], 'text' => $request->text[$key]]);
        }
    ...
    public function simple_update($category, $translate_model, $images_model, $request, $id)
    {
        ...
        // Update translations
        foreach ($request->title as $key => $value) {
            $translation = $translate_model::where('team_id', $id)->where('lang_key', $key)->first();
            if ($translation) {
                $translation->update(['team_id' => $id,'lang_key' => $key, 'title' => $value, 'position' => $request->position[$key], 'text' => $request->text[$key]]);
            } else {
                $translate_model::create(['team_id' => $id,'lang_key' => $key, 'title' => $value, 'position' => $request->position[$key], 'text' => $request->text[$key]]);
            }
        }
    ...
    public function handleImages($request, $images_model, $category_id, $directory, $type)
    {
        if ($request->hasFile($type)) {
            foreach ($request->file($type) as $image) {
                ...
                $images_model::create(['image_url' => $imagePath, 'team_id' => $category_id, 'model_type' => $type]);
            }
        }
    }
