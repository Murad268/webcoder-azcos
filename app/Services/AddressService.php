...
class AddressService
{
    public function simple_create($main_model, $translate_model, $request)
    {
        ...
        foreach ($request->title as $key => $value) {
            $translate_model::create(['address_id' => $translate->id, 'lang_key' => $key, 'title' => $value]);
        }
    ...
    public function simple_update($category, $translate_model,  $request, $id)
    {
        ...
        // Update translations
        foreach ($request->title as $key => $value) {
            $translation = $translate_model::where('address_id', $id)->where('lang_key', $key)->first();
            if ($translation) {
                $translation->update(['title' => $value]);
            } else {
                $translate_model::create(['address_id' => $id, 'lang_key' => $key, 'title' => $value]);
            }
        }
