    ...
    <div id="tableExample4" data-list='{"valueNames":["name","email","payment"],"page":5,"pagination":true,"filter":{"key":"payment"}}'>
        ...
        <div class="table-responsive">
            <div style="column-gap: 10px" class="d-flex">
                <a class="badge badge-phoenix badge-phoenix-primary mb-3" style="padding: 7px" href="{{route('admin.color_schemes.create', TranslateUtility::getLang())}}">{{TranslateUtility::getTranslate('admin_index', 'add', TranslateUtility::getLang())}}</a>
                <a data-link="{{route('admin.color_shemes.delete_selected_colors')}}" class="badge badge-phoenix badge-phoenix-danger mb-3 delete-all" style="padding: 7px; cursor: pointer; text-decoration : none">{{TranslateUtility::getTranslate('admin_index', 'delete_selected',TranslateUtility::getLang())}}</a>
                <div>
                    <form>
                    ...
                    <tr>
                        ...
                        <td class="align-middle ps-3 name">
                            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(40px, 1fr)); gap: 5px">
                                @foreach($category->images->where('model_type', 'images') as $image)
                                <div class="image_block" style="width: 40px; height: 40px;">
                                    <a href="{{ route('admin.color_schemes.delete_image', ['id' => $image->id]) }}" class="delete_image">
                                        &#x2715;
                                    </a>
                        ...
                        <td class="align-middle ps-3 name">
                            <form class="d-flex" style="width: 400px; column-gap: 5px" enctype="multipart/form-data" method="post" action="{{route('admin.color_schemes.add_images', ['type' => 'images', 'id' => $category->id])}}">
                                @csrf
                                <div class="mb-1">
                        ...
                        <td class="align-middle ps-3 name">{{$category->getWithLocale($lang) ? $category->getWithLocale($lang)->title: "" }}</td>
                        ...
                        <td class="align-middle ps-3 name">
                            <a href="{{route('admin.color_schemes.edit', ['lang' => $lang, 'color_scheme' => $category->id])}}" class="btn btn-phoenix-success me-1 mb-1" type="button"><i class="fa fa-pencil"></i></a>
                            ...
                            @if($activeCount < 2 or $categories->count() < 2) <a style="cursor: none" class="btn btn-phoenix-danger me-1 mb-1 change_status_false" type="button"><i class="fas fa-info" aria-hidden="true"></i></a>
                                    @else
                                    <a href="{{route('admin.color_schemes.changeStatusFalse', $category->id)}}" class="btn btn-phoenix-secondary me-1 mb-1 change_status_false" type="button"><i class="fas fa-lock-open" aria-hidden="true"></i></a>
                                    @endif
                                    @else
                                    <a href="{{route('admin.color_schemes.changeStatusTrue', $category->id)}}" class="btn btn-phoenix-secondary me-1 mb-1 change_status_true" type="button"><i class="fas fa-lock" aria-hidden="true"></i></a>
                                    @endif
                        </td>
                    </tr>
                    @endforeach
...
<script>
    ...
    Sortable.create(tbody, {
        ...
        onEnd: function(evt) {
            ...
            fetch(`{{ url('/api/admin/color_shemes/changeOrder') }}`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
