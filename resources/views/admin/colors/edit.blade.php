...
<div class="p-4 code-to-copy">
    ...
    <form enctype="multipart/form-data" method="post" action="{{route('admin.color_schemes.update', ['lang' => TranslateUtility::getLang(), 'color_scheme' => $category->id])}}">
        @method('PATCH')
        @csrf
                    ...
                    <div class="input_group_first">
                        <label class="file_inp w-100" for="file">
                            ...
                            <div class="file_container">
                                ...
                                <div class="drop_file d-flex align-items-center mt-1">
                                    @foreach($category->images as $image)
                                        <div style="position: relative; display: inline-block;">
                                            <img src="{{ asset('storage/' . $image->image_url) }}" style="max-width: 100px; margin: 5px;">
                                            <button style="position: absolute; top: 0; right: 0; background: red; color: white; border: none; border-radius: 50%; cursor: pointer;" onclick="removeImage(this, '{{ $image->id }}')">×</button>
                ...
                <div class="tab-content">
                    ...
                    <div class="tab-pane fade @if($loop->first) show active @endif" id="content-{{ $lang->code }}" role="tabpanel" aria-labelledby="tab-{{ $lang->code }}">
                        <div class="mb-6 input_group_second">
                            <input name="title[{{ $lang->code }}]" class="form-control @error('title.' . $lang->code) is-invalid @enderror" type="text" placeholder="{{TranslateUtility::getTranslate('admin_form', 'color_name', $lang->code)}}" value="{{ old('locale.' . $lang->code, $category->getWithLocale($lang->code)->title ?? '') }}" />
        ...
        <button style="margin-top: -46px;" class="btn btn-success" type="submit">{{TranslateUtility::getTranslate('admin_form', 'update_button', $lang->code)}}</button>
    </form>
</div>
