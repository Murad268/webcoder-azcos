    ...
    <div id="tableExample4" data-list='{"valueNames":["name","email","payment"],"page":5,"pagination":true,"filter":{"key":"payment"}}'>
        ...
        <div class="table-responsive">
            <div style="column-gap: 10px" class="d-flex">
                <a class="badge badge-phoenix badge-phoenix-primary mb-3" style="padding: 7px" href="{{route('admin.categories.create', $lang)}}">{{TranslateUtility::getTranslate('admin_index', 'add',TranslateUtility::getLang())}}</a>
                <a data-link="{{route('admin.categories.delete_selected_categories')}}" class="badge badge-phoenix badge-phoenix-danger mb-3 delete-all" style="padding: 7px; cursor: pointer; text-decoration : none">{{TranslateUtility::getTranslate('admin_index', 'delete_selected', TranslateUtility::getLang())}}</a>
                <div>
                    <form>
            ...
            <table class="table table-sm fs-9 mb-0">
                <thead>
                    <tr class="bg-body-highlight">
                        ...
                        <th class=" border-top border-translucent">{{TranslateUtility::getTranslate('admin_index', 'banners', TranslateUtility::getLang())}}</th>
                        <th class=" border-top border-translucent">{{TranslateUtility::getTranslate('admin_index', 'add_image', TranslateUtility::getLang())}}</th>
                ...
                <tbody class="list">
                    ...
                    <tr>
                        ...
                        <td class="align-middle ps-3 name">
                            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(40px, 1fr)); gap: 5px">
                                ...
                                <div class="image_block" style="width: 40px; height: 40px;">
                                    <a href="{{ route('admin.categories.delete_image', ['id' => $image->id]) }}" class="delete_image">
                                        &#x2715;
                                    </a>
                        ...
                        <td class="align-middle ps-3 name">
                            <form class="d-flex" style="width: 400px; column-gap: 5px" enctype="multipart/form-data" method="post" action="{{route('admin.categories.add_images', ['type' => 'images', 'id' => $brand->id])}}">
                                @csrf
                                <div class="mb-1">
                        ...
                        <td class="align-middle ps-3 name">
                            <form class="d-flex" style="width: 400px; column-gap: 5px" enctype="multipart/form-data" method="post" action="{{route('admin.categories.add_images', ['type' => 'banners', 'id' => $brand->id])}}">
                                @csrf
                                <div class="mb-1">
                                    <input class="form-control" name="banners[]" multiple="multiple" type="file">
                                </div>
                                <button class="btn btn-success" type="submit">{{TranslateUtility::getTranslate('admin_index', 'add_image', TranslateUtility::getLang())}}</button>
                            </form>
                        </td>
                        ...
                        <td class="align-middle ps-3 name">{{$brand->getWithLocale($lang) ? $brand->getWithLocale($lang)->title: "" }}</td>
                        ...
                        <td class="align-middle ps-3 name">
                            <a href="{{route('admin.categories.edit', ['lang' => TranslateUtility::getLang(), 'category' => $brand->id])}}" class="btn btn-phoenix-success me-1 mb-1" type="button"><i class="fa fa-pencil"></i></a>
                            ...
                            @if($activeCount < 2 or $categories->count() < 2) <a style="cursor: none" class="btn btn-phoenix-danger me-1 mb-1 change_status_false" type="button"><i class="fas fa-info" aria-hidden="true"></i></a>
                                    @else
                                    <a href="{{route('admin.categories.changeStatusFalse', $brand->id)}}" class="btn btn-phoenix-secondary me-1 mb-1 change_status_false" type="button"><i class="fas fa-lock-open" aria-hidden="true"></i></a>
                                    @endif
                                    @else
                                    <a href="{{route('admin.categories.changeStatusTrue', $brand->id)}}" class="btn btn-phoenix-secondary me-1 mb-1 change_status_true" type="button"><i class="fas fa-lock" aria-hidden="true"></i></a>
                                    @endif
                        </td>
                    </tr>
                    @endforeach
...
<script>
    ...
    Sortable.create(tbody, {
        ...
        onEnd: function(evt) {
            ...
            fetch(`{{ url('/api/admin/categories/changeOrder') }}`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
