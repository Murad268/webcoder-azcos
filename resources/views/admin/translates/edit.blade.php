...
<div class="p-4 code-to-copy">
    <form method="post" action="{{route('admin.translates.update', ['translate' => $translate->id, 'lang' => $lang])}}">
        @method('PATCH')
        @csrf
            ...
            <div class="d-flex" style="column-gap: 12px">
                <div style="flex:6">
                    <div class="input_group_first">
                        <input name="group" class="form-control @error('group') is-invalid @enderror" id="exampleFormControlInput" type="text" placeholder="Tərcümə qrupu" value="{{ old('group', $translate->group) }}" />
                        @error('group')
                ...
                <div style="flex:6">
                    <div class="input_group_first">
                        <input name="code" class="form-control @error('code') is-invalid @enderror" id="exampleFormControlInput" type="text" placeholder="Tərcümə açar sözü" value="{{ old('code', $translate->code) }}" />
                        @error('code')
                        <div class="invalid-feedback">
                            {{ $message }}
                        </div>
                ...
                <div class="tab-content">
                    ...
                    <div class="tab-pane fade @if($loop->first) show active @endif" id="content-{{ $lang->code }}" role="tabpanel" aria-labelledby="tab-{{ $lang->code }}">
                        <div class="mb-6 input_group_second">
                            <input name="locale[{{ $lang->code }}]" class="form-control @error('locale.' . $lang->code) is-invalid @enderror" type="text" placeholder="Tərcümə {{ $lang->code }}" value="{{ old('locale.' . $lang->code, $translate->getWithLocale($lang->code, 'value')) }}" />
