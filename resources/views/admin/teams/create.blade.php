...
@section('content')
    <div class="p-4 code-to-copy">
        <form enctype="multipart/form-data" method="post" action="{{ route('admin.team.store', TranslateUtility::getLang()) }}">
            @csrf
                        ...
                        @foreach($langs as $lang)
                            <div class="tab-pane fade @if($loop->first) show active @endif" id="title-content-{{ $lang->code }}" role="tabpanel" aria-labelledby="title-tab-{{ $lang->code }}">
                                <div class="mb-6 input_group_second">
                                    <input name="title[{{ $lang->code }}]" class="form-control @error('title.' . $lang->code) is-invalid @enderror" type="text" placeholder="{{TranslateUtility::getTranslate('admin_form', 'team_member_name', $lang->code)}}" value="{{ old('title.' . $lang->code) }}" />
                    ...
                    <div class="tab-content">
                        @foreach($langs as $lang)
                            <div class="tab-pane fade @if($loop->first) show active @endif" id="text-content-{{ $lang->code }}" role="tabpanel" aria-labelledby="text-tab-{{ $lang->code }}">
                                <div class="mb-6 input_group_second">
                                    <textarea placeholder="{{TranslateUtility::getTranslate('admin_form', 'team_member_about', $lang->code)}}" id="summernote-text-{{ $lang->code }}" name="text[{{ $lang->code }}]" class="form-control @error('text.' . $lang->code) is-invalid @enderror">{{ old('text.' . $lang->code) }}</textarea>
                    ...
                    <div class="tab-content">
                        @foreach($langs as $lang)
                            <div class="tab-pane fade @if($loop->first) show active @endif" id="position-content-{{ $lang->code }}" role="tabpanel" aria-labelledby="position-tab-{{ $lang->code }}">
                                <div class="mb-6 input_group_second">
                                    <input name="position[{{ $lang->code }}]" class="form-control @error('position.' . $lang->code) is-invalid @enderror" type="text" placeholder="{{TranslateUtility::getTranslate('admin_form', 'team_member_position', $lang->code)}}" value="{{ old('position.' . $lang->code) }}" />
...
@section('scripts')
    ...
    <script>
        $(document).ready(function() {
            ...
            @foreach($langs as $lang)
            initializeSummernote('#summernote-text-{{ TranslateUtility::getLang() }}');
            @endforeach
