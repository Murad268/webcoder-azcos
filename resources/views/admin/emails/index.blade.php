    ...
    <div id="tableExample4" data-list='{"valueNames":["name","email","payment"],"page":5,"pagination":true,"filter":{"key":"payment"}}'>
        ...
        <div class="table-responsive">
            <div style="column-gap: 10px" class="d-flex">
                <a class="badge badge-phoenix badge-phoenix-primary mb-3" style="padding: 7px" href="{{route('admin.email.create', TranslateUtility::getLang())}}">{{TranslateUtility::getTranslate('admin_index', 'add', TranslateUtility::getLang())}}</a>
                <a data-link="{{route('admin.email.delete_selected_emails')}}" class="badge badge-phoenix badge-phoenix-danger mb-3 delete-all" style="padding: 7px; cursor: pointer; text-decoration : none">{{TranslateUtility::getTranslate('admin_index', 'delete_selected',TranslateUtility::getLang())}}</a>
                <div>
                    <form>
            ...
            <table class="table table-sm fs-9 mb-0">
                <thead>
                    <tr class="bg-body-highlight">
                        ...
                        <th class=" border-top border-translucent">{{TranslateUtility::getTranslate('admin_index', '$email',TranslateUtility::getLang())}}</th>
                    ...
                    <tr>
                        ...
                        <td class="align-middle ">
                            ...
                            <div class="form-check">
                                <input data-id='{{$email->id}}' class="form-check-input select-email" id="flexCheckChecked" type="checkbox" value="" />
                            </div>
                            @else
                            <input style="display: none" data-id='{{$email->id}}' class="form-check-input select-email" id="flexCheckChecked" type="checkbox" value="" />
                            @endif
                        </td>

                        <td class="align-middle ps-3 name">{{$email->data}}</td>
                        ...
                        <td>
                            <div class="d-flex">
                                <a href="{{route('admin.email.edit',['email'=>$email->id, 'lang' => $lang] )}}" class="btn btn-phoenix-success me-1 mb-1" type="button"><i class="fa fa-pencil"></i></a>

                                @if($email->status)
                                        @if($activeemailsCount < 2 or $emails->count() < 2)
                                        ...
                                        @else
                                        <a href="{{route('admin.email.changeStatusFalse', $email->id)}}" class="btn btn-phoenix-secondary me-1 mb-1 change_status_false" type="button"><i class="fas fa-lock-open" aria-hidden="true"></i></a>
                                        @endif

                                @else
                                <a href="{{route('admin.email.changeStatusTrue', $email->id)}}" class="btn btn-phoenix-secondary me-1 mb-1 change_status_true" type="button"><i class="fas fa-lock" aria-hidden="true"></i></a>
                                @endif
                            </div>
                        </td>
                    </tr>
...
<script>
    let selectedLangs  = [];
    document.querySelectorAll('.select-email').forEach(checkbox => {
    ...
    Sortable.create(tbody, {
        ...
        onEnd: function(evt) {
            ...
            for (let i = 0; i < items.length; i++) {
                const dataId = items[i].querySelector('.select-email').getAttribute('data-id');
                emails.push({
                    id: dataId,
                    order: i + 1
                });
            ...
            // Send the entire list of emails with their new order to the API
            fetch(`{{ url('/api/admin/email/changeOrder') }}`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
