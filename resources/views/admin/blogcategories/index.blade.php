    ...
    <div id="tableExample4" data-list='{"valueNames":["name","email","payment"],"page":5,"pagination":true,"filter":{"key":"payment"}}'>
        ...
        <div class="table-responsive">
            <div style="column-gap: 10px" class="d-flex">
                <a class="badge badge-phoenix badge-phoenix-primary mb-3" style="padding: 7px" href="{{route('admin.blog_category.create', TranslateUtility::getLang())}}">{{TranslateUtility::getTranslate('admin_index', 'add',  TranslateUtility::getLang())}}</a>
                <a data-link="{{route('admin.blog_category.delete_selected_categories')}}" class="badge badge-phoenix badge-phoenix-danger mb-3 delete-all" style="padding: 7px; cursor: pointer; text-decoration : none">{{TranslateUtility::getTranslate('admin_index', 'delete_selected', TranslateUtility::getLang())}}</a>
                <div>
                    <form>
            ...
            <table class="table table-sm fs-9 mb-0">
                ...
                <tbody class="list">
                    ...
                    <tr>
                        ...
                        <td class="align-middle ps-3 name">{{$brand->getWithLocale( TranslateUtility::getLang()) ? $brand->getWithLocale( TranslateUtility::getLang())->slug: "" }}</td>
                        ...
                        <td class="align-middle ps-3 name">
                            <a href="{{route('admin.blog_category.edit', ['lang' =>  TranslateUtility::getLang(), 'blog_category' => $brand->id])}}" class="btn btn-phoenix-success me-1 mb-1" type="button"><i class="fa fa-pencil"></i></a>
                            ...
                            @if($activeCount < 2 or $categories->count() < 2) <a style="cursor: none" class="btn btn-phoenix-danger me-1 mb-1 change_status_false" type="button"><i class="fas fa-info" aria-hidden="true"></i></a>
                                    @else
                                    <a href="{{route('admin.blog_category.changeStatusFalse', $brand->id)}}" class="btn btn-phoenix-secondary me-1 mb-1 change_status_false" type="button"><i class="fas fa-lock-open" aria-hidden="true"></i></a>
                                    @endif
                                    @else
                                    <a href="{{route('admin.blog_category.changeStatusTrue', $brand->id)}}" class="btn btn-phoenix-secondary me-1 mb-1 change_status_true" type="button"><i class="fas fa-lock" aria-hidden="true"></i></a>
                                    @endif
                        </td>
                    </tr>
                    @endforeach
            ...
            if (e.target.getAttribute("href")) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        Swal.fire(result.error, "", "success")
                    }
                });
            }
        });
