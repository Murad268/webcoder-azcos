...
@section('content')
    ...
    <div class="p-4 code-to-copy">
        ...
        <form enctype="multipart/form-data" method="post" action="{{route('admin.tag.update', ['lang' => $lang, 'tag' => $category->id])}}">
            @method('PATCH')
            @csrf
                        ...
                        @foreach($langs as $lang)
                            <div class="tab-pane fade @if($loop->first) show active @endif" id="content-{{ $lang->code }}" role="tabpanel" aria-labelledby="tab-{{ $lang->code }}">
                                <div class="mb-6 input_group_second">
                                    <input name="tag[{{ $lang->code }}]" class="form-control @error('title.' . $lang->code) is-invalid @enderror" type="text" placeholder="{{TranslateUtility::getTranslate('admin_form', 'tag', $lang->code)}}" value="{{ old('locale.' . $lang->code, $category->getWithLocale($lang->code)->tag ?? '') }}" />
